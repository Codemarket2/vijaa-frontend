...
import {
  ...
  GET_LIST_ITEM_WITH_FIELD_BY_SLUG,
...
export function useGetListItemWithFieldBySlug({ slug }: any) {
  const { data, error, loading } = useQuery(GET_LIST_ITEM_WITH_FIELD_BY_SLUG, {
    variables: { slug },
    fetchPolicy: 'cache-and-network',
  });
